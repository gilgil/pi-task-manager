# Wiki Index

> Auto-generated from meta/registry.json. Do not edit manually.

## Concepts

- [[concepts/dataclass-porting]] — Dataclass Porting *(created: 2026-08-14)*
- [[concepts/date-fields]] — Date fields *(created: 2026-08-14)*
- [[concepts/emoji-annotations]] — Emoji Annotations *(created: 2026-08-14)*
- [[concepts/on-completion]] — On Completion *(created: 2026-08-14)*
- [[concepts/priority-levels]] — Priority levels *(created: 2026-08-14)*
- [[concepts/python-to-typescript-port]] — Python to TypeScript Port *(created: 2026-08-14)*
- [[concepts/recurrence]] — Recurrence *(created: 2026-08-14)*
- [[concepts/task-dependencies]] — Task Dependencies *(created: 2026-08-14)*
- [[concepts/task-hierarchy]] — Task Hierarchy *(created: 2026-08-14)*
- [[concepts/task-metadata]] — Task Metadata *(created: 2026-08-14)*
- [[concepts/tasks-emoji-format]] — Tasks Emoji Format *(created: 2026-08-14)*
- [[concepts/todomd-format]] — TODO.md Format *(created: 2026-08-14)*

## Entitys

- [[entities/id-generation]] — ID Generation *(created: 2026-08-14)*
- [[entities/parser]] — Parser *(created: 2026-08-14)*
- [[entities/task]] — Task *(created: 2026-08-14)*
- [[entities/task-data-model]] — Task data model *(created: 2026-08-14)*
- [[entities/taskmanager-class]] — TaskManager class *(created: 2026-08-14)*
- [[entities/tasks-plugin]] — Tasks plugin *(created: 2026-08-14)*
- [[entities/tool-definitions]] — Tool Definitions *(created: 2026-08-14)*

## Sources

- [[sources/obs-2026-08-14-wiki-initialized-for-pi-task-manager-project]] — Observation: Wiki initialized for pi-task-manager project *(created: 2026-08-14)*
- [[sources/obs-2026-08-15-pi-task-manager-code-review-19-tasks-added-under-investigate]] — Observation: pi-task-manager code review: 19 tasks added under "Investigate & Fix" *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-task-manager-flat-list-refactored-to-true-tree]] — Observation: pi-task-manager: flat list refactored to true tree *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-task-manager-input-validation-added-to-addtask-edittask-t]] — Observation: pi-task-manager: input validation added to addTask/editTask (TDD) *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-task-manager-installed-and-verified-in-real-pi]] — Observation: pi-task-manager installed and verified in real pi *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-task-manager-todo-md-ported-from-flat-to-tree-format]] — Observation: pi-task-manager TODO.md ported from flat to tree format *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-tool-execute-sync-return-works-at-runtime-but-fails-tsc-s]] — Observation: pi tool execute: sync return works at runtime but fails tsc strict *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-pi-tui-labels-local-path-extensions-by-shortest-unique-path-]] — Observation: pi TUI labels local-path extensions by shortest unique path suffix *(created: 2026-08-15)*
- [[sources/obs-2026-08-15-session-start-hook-added-to-pi-task-manager]] — Observation: session_start hook added to pi-task-manager *(created: 2026-08-15)*
- [[sources/SRC-2026-08-14-001]] — task_manager.py *(created: unknown)*
- [[sources/SRC-2026-08-14-002]] — task_manager_design.md *(created: unknown)*
- [[sources/SRC-2026-08-14-003]] — tasks_emojis_format.md *(created: unknown)*
- [[sources/SRC-2026-08-14-004]] — PLAN.md *(created: unknown)*
- [[sources/SRC-2026-08-14-005]] — task.ts *(created: unknown)*
- [[sources/SRC-2026-08-14-006]] — task-manager.ts *(created: unknown)*
- [[sources/SRC-2026-08-14-007]] — tools.ts *(created: unknown)*
- [[sources/wiki-initialization-complete]] — pi-task-manager wiki initialized and populated *(created: 2026-08-14)*

---
*Last updated: 2026-08-15T18:05:17.129Z* | *Total pages: 36*
